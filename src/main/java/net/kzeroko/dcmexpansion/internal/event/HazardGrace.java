...
public class HazardGrace {
    @SubscribeEvent
    public static void onConditionHazardGrace(EntityJoinLevelEvent event) {
        if (!event.getLevel().isClientSide() && event.getEntity() instanceof Player player) {
            if (player.level().getBiome(player.getOnPos()).is(DcmTags.HAZARD_BIOMES)) {
                player.addEffect(new MobEffectInstance(DcmEffects.HAZARD_GRACE.get(),
                        30 * 20, 0, false, false, true));
            }
        }
    }
}